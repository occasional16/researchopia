...
### 4. 创建 Changeset (针对 npm 包)

当 `packages/shared` 等 npm 包需要发布时,请使用 Changesets:

```bash
# 交互式选择包并填写变更说明
npm run changeset

# 在 PR 合并或发布前更新版本号和CHANGELOG
...
执行后,仓库会生成 `.changeset/*.md` 变更记录与自动更新的 `package.json`/`CHANGELOG.md`。
...
| 命令 | 说明 |
|------|------|
| `npm run changeset` | 交互式创建变更,生成 `.changeset/XXXX.md` |
| `npm run release:version` | 根据累计变更提高版本号并更新Changelog |
| `npm run release:publish` | 将变更发布到 npm (需要 `npm auth`) |

配置文件位于 `.changeset/config.json`, 默认 `baseBranch = main`, `access = restricted`, 适用于内部包发布。
...
- [docs/README.md](../README.md) - 文档索引和规范
