...
import PaperBookmarkModal from './PaperBookmarkModal'
...
/**
 * Universal paper bookmark button component
 ...
 */
export default function PaperBookmarkButton({
  paperId,
  doi,
  title,
  variant = 'icon',
  ...
  // Check bookmark status on mount and when paper changes
  ...
  }, [user, paperId, getAccessToken, authLoading])
      ...
      return (
        ...
        >
          ...
          <span className="text-sm">
            {isBookmarked ? `已收藏${bookmarkCount > 1 ? ` (${bookmarkCount})` : ''}` : '收藏'}
          </span>
        </button>
      )
    }
    ...
    return (
      ...
      >
        {loading ? (
          ...
          <BookmarkCheck size={14} className="fill-yellow-500" />
        ) : (
          <Bookmark size={14} />
        )}
