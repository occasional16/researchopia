...
interface UserFavoritesProps {
  username?: string  // 如果提供，则查看指定用户的收藏；否则查看当前用户的收藏
}

export default function UserFavorites({ username }: UserFavoritesProps = {}) {
  ...
  const loadFavorites = async () => {
    try {
      ...
      if (username) {
        ...
        const response = await fetch(`/api/users/${username}/favorites?limit=100`, {
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-500 mb-6">
          {isCurrentUser ? '您还没有收藏任何论文' : '该用户还没有收藏任何论文'}
        </p>
        {isCurrentUser && (
          <Link
            href="/search"
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-medium text-gray-900">
          {isCurrentUser ? '我的收藏' : '收藏的论文'} ({favorites.length})
        </h3>
      </div>
