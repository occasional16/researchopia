...
export class MessageHelper {
            ...
            this.listener = async (event) => {
                ...
                if (!handler) {
                    ...
                    if (!_handlerName.endsWith("::return")) {
                        logError(`Handler ${_handlerName} not found for ${this.config.name}`);
                    }
                    return;
                }
                try {
    ...
    async exec(name, params, options = {}) {
        ...
        // Send the message
        const jobID = await this.send({ name: name, data: params || [] });
    ...
    async send(options) {
        ...
        if (this.config.dev) {
            logError(`Sending message ${name} from ${this.config.name}, ${new Date().toISOString()}`);
        }
        const message = {
            _jobID: jobID,
    ...
    async isTargetAlive() {
        ...
        if (typeof this.target.closed !== "undefined") {
            // Window
