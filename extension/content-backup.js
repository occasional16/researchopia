...
class ResearchopiaContentScript {
  ...
  setupFloatingIconEvents(icon) {
    ...
    console.log('🎛️ 图标对象:', icon);
    console.log('🎛️ 图标类名:', icon.className);
    ...
    icon.addEventListener('mousedown', (e) => {
      ...
      // 在开始拖拽时移除transform，设置绝对位置
      icon.style.transform = 'none';
      icon.style.left = `${rect.left}px`;
      icon.style.top = `${rect.top}px`;
      icon.style.right = 'auto';
      icon.style.bottom = 'auto';
      console.log('🎯 转换为绝对定位:', rect.left, rect.top);
    ...
    this.globalMouseMove = (e) => {
      if (this.isDragging && icon) {
        ...
        icon.style.right = 'auto';
        icon.style.bottom = 'auto';
        icon.style.transform = 'none'; // 移除可能的transform影响
        
        console.log('🖱️ 拖拽移动到:', finalX, finalY);
        console.log('🎯 当前样式 left:', icon.style.left, 'top:', icon.style.top);
        e.preventDefault();
      }
    };
